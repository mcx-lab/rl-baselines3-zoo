...
class FootStepper(object):
    ...
    def get_reference_pos_swing_foot(self):
        self.new_pos_local = np.array(self.toe_pos_local_ref[self.swing_foot_index])
    ...
    def update(self, step_input):
        ...
        for i in range(len(self.toe_pos_local_ref)):
            ...
            toe = [
                toe[0] + self.diff_world[0],
                toe[1] + self.diff_world[1],
                toe[2] + self.diff_world[2],
            ]
            self.toe_pos_local_ref[i] = toe

        self.local_diff_y = (
            self.toe_pos_local_ref[0][1]
            + self.toe_pos_local_ref[1][1]
            - self.toe_pos_local_ref[2][1]
            - self.toe_pos_local_ref[3][1]
        )
