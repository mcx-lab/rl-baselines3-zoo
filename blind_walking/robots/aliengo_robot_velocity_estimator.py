...
# should be for real robot application. Only changed name from a1 to aliengo. 

class VelocityEstimator:
    """Estimates base velocity of Aliengo robot.
    ...
    def _compute_delta_time(self, robot_state):
        if self._last_timestamp == 0.0:
            ...
            delta_time_s = (robot_state.tick - self._last_timestamp) / 1000.0
    ...
    def update(self, robot_state):
        ...
        rot_mat = self.robot.pybullet_client.getMatrixFromQuaternion(base_orientation)
        rot_mat = np.array(rot_mat).reshape((3, 3))
        calibrated_acc = rot_mat.dot(sensor_acc) + np.array([0.0, 0.0, -9.8])
