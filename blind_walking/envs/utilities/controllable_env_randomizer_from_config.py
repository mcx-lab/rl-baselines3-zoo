...
class ControllableEnvRandomizerFromConfig(
    ...
    def randomize_env(self, env):
        ...
        if not self.suspend_randomization:
            ...
            self._randomization_function_dict = self._build_randomization_func_dict(env)
    ...
    def _randomize_masses(self, minitaur, lower_bound, upper_bound, parameters=None):
        if parameters is None:
            sample = self._np_random.uniform(
                [self._param_bounds[0]] * 2, [self._param_bounds[1]] * 2
            )
        else:
            sample = parameters
    ...
    def _randomize_individual_masses(
        self, minitaur, lower_bound, upper_bound, parameters=None
    ):
        base_mass = minitaur.GetBaseMassesFromURDF()
        leg_masses = minitaur.GetLegMassesFromURDF()
        ...
        if parameters is None:
            sample = self._np_random.uniform(
                [self._param_bounds[0]] * param_dim, [self._param_bounds[1]] * param_dim
            )
        else:
            sample = parameters
    ...
    def _randomize_individual_inertia(
        self, minitaur, lower_bound, upper_bound, parameters=None
    ):
        ...
        random_base_ratio = np.reshape(
            randomized_inertia_ratios[0 : len(base_inertia) * 3], (len(base_inertia), 3)
        )
        ...
        random_leg_ratio = np.reshape(
            randomized_inertia_ratios[len(base_inertia) * 3 :], (len(leg_inertia), 3)
        )
    ...
    def _randomize_contact_restitution(
        self, minitaur, lower_bound, upper_bound, parameters=None
    ):
        if parameters is None:
            sample = self._np_random.uniform(
    ...
    def _randomize_contact_friction(
        self, minitaur, lower_bound, upper_bound, parameters=None
    ):
        if parameters is None:
            sample = self._np_random.uniform(
    ...
    def _randomize_global_motor_strength(
        self, minitaur, lower_bound, upper_bound, parameters=None
    ):
        if parameters is None:
            sample = self._np_random.uniform(
        ...
        minitaur.SetMotorStrengthRatios(
            [randomized_motor_strength_ratio] * minitaur.num_motors
        )
        if self._verbose:
            logging.info(
                "global motor strength is: {}".format(randomized_motor_strength_ratio)
            )
    ...
    def _randomize_leg_weakening(
        ...
        if parameters is None:
            ...
            normalized_ratio = self._np_random.uniform(
                self._param_bounds[0], self._param_bounds[1]
            )
        ...
        motor_strength_ratios[
            leg_to_weaken * motor_per_leg : (leg_to_weaken + 1) * motor_per_leg
        ] = leg_weaken_ratio
        minitaur.SetMotorStrengthRatios(motor_strength_ratios)
        if self._verbose:
            logging.info(
                "weakening leg {} with ratio: {}".format(
                    leg_to_weaken, leg_weaken_ratio
                )
            )

    def _randomize_single_leg_weakening(
        self, minitaur, lower_bound, upper_bound, parameters=None
    ):
        motor_per_leg = int(minitaur.num_motors / NUM_LEGS)
        leg_to_weaken = 0
