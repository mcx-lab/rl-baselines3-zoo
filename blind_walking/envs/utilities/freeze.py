...
class freeze_aliengo_gym_env:
    """Provides a context to freeze an AliengoGymEnv-v0 environment.
    This context allows the user to manipulate the state of the AliengoGymEnv-v0 environment and return it
    to its original state upon exiting the context.
    ...
    .. code-block:: python
       ...
       action = env.action_space.sample()
       # o1_expected, *_ = env.step(action)
       with freeze_aliengo_gym_env(env):
           step_the_env_a_bunch_of_times()
       o1, *_ = env.step(action) # o1 will be equal to what o1_expected would have been
    Args:
        env: the environment to freeze.
