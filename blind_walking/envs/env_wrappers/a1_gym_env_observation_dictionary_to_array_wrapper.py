...
class A1GymEnvObservationDictionaryToArrayWrapper(gym.Env):
  ...
  def _flatten_observation_spaces(self, observation_spaces):
    flattened_robot_observations = env_utils.flatten_observation_spaces(observation_spaces.spaces['robot_observations'])
    flattened_environment_state = env_utils.flatten_observation_spaces(observation_spaces.spaces['environment_state'])

    return spaces.dict.Dict({
        'robot_observations': flattened_robot_observations,
        'environment_state': flattened_environment_state
    })

  def _flatten_observation(self, input_observation):
    """Flatten the dictionary to an array."""
    flattened_robot_observations = env_utils.flatten_observations(input_observation['robot_observations'])
    flattened_environment_state = env_utils.flatten_observations(input_observation['environment_state'])
