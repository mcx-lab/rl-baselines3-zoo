...
class Stairs(EnvModifier):
    ...
    def _generate(
        self, env, start_x=3, num_steps=5, step_rise=0.1, step_run=0.2, friction=0.5
    ):
        ...
        for i in range(num_steps):
            ...
            env.pybullet_client.changeVisualShape(
                step, -1, rgbaColor=colors[i % len(colors)]
            )
            base_pos = [sum(x) for x in zip(base_pos, [step_run, 0, step_rise])]

        # Create downwards stairs
        base_pos = [
            sum(x) for x in zip(base_pos, [boxHalfLength * 2 - step_run, 0, -step_rise])
        ]
        for i in range(num_steps):
            step = env.pybullet_client.createMultiBody(
                baseMass=0,
            ...
            env.pybullet_client.changeDynamics(step, -1, lateralFriction=friction)
            env.pybullet_client.changeVisualShape(
                step, -1, rgbaColor=colors[(-i - 1) % len(colors)]
            )
            base_pos = [sum(x) for x in zip(base_pos, [step_run, 0, -step_rise])]
