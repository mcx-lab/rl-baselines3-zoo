...
def main():  # noqa: C901
    ...
    modification_suffix = "modified" if args.modify_observation else "unmodified"
    ...
    terrain_modifier_to_replay_path_map = {}
    stats_path = Path(log_path) / "stats"
    stats_path.mkdir(exist_ok=True, parents=True)

    # For HER, monitor success rate
    successes = []
    sensor_names = [f"{w}_flatten" for w in ["depthfr", "depthfl", "depthrr", "depthrl", "depthmiddle"]]

    try:
        for episode, terrain_modifier in enumerate(args.terrain_modifiers):
            print(f"Starting episode {episode} with terrain {terrain_modifier}")
            if args.record:
                replay_path = get_video_path(env)
                terrain_modifier_to_replay_path_map[terrain_modifier] = replay_path

            raw_loggers = {}
            filtered_loggers = {}
            filters = {}
            for sensor_name in sensor_names:
                raw_loggers[sensor_name] = Logger(f"raw_{sensor_name}_{terrain_modifier}_{modification_suffix}")
                filtered_loggers[sensor_name] = Logger(f"filtered_{sensor_name}_{terrain_modifier}_{modification_suffix}")
                start_idx, end_idx = env.env_method("get_sensor_start_end_index", sensor_name)[0]
                sensor_dim = end_idx - start_idx
                filters[sensor_name] = ActionFilterButter(
                    lowcut=[0], highcut=[6.0], order=1, sampling_rate=1 / (0.001 * 33), num_joints=sensor_dim
                )
            ...
            for modifiers in modifierss:
                ...
                for modifier in modifiers:
                    modifier._override_reset(terrain_modifier)
            ...
            for _ in range(args.n_timesteps):
                ...
                for sensor_name in sensor_names:
                    start_idx, end_idx = env.env_method("get_sensor_start_end_index", sensor_name)[0]
                    raw_sensor_obs = obs[0, start_idx:end_idx]
                    filtered_sensor_obs = filters[sensor_name].filter(raw_sensor_obs)
                    raw_loggers[sensor_name].update(raw_sensor_obs)
                    filtered_loggers[sensor_name].update(filtered_sensor_obs)

                    if args.modify_observation:
                        obs[0, start_idx:end_idx] = filtered_sensor_obs
            ...
            for sensor_name in sensor_names:
                raw_loggers[sensor_name].save(stats_path)
                filtered_loggers[sensor_name].save(stats_path)

    except KeyboardInterrupt:
        pass

    with open(os.path.join(log_path, f"env_modifier_to_replay_path_map_{modification_suffix}.json"), "w") as jsonfile:
        json.dump(terrain_modifier_to_replay_path_map, jsonfile)
