...
flags.DEFINE_enum("motor_control_mode", "Torque", ["Torque", "Position", "Hybrid"], "Motor Control Mode.")
...
def main(_):
    ...
    for dim in range(dim_action):
        action_selector_id = p.addUserDebugParameter(
            paramName="dim{}".format(dim), rangeMin=action_low[dim], rangeMax=action_high[dim], startValue=action_median[dim]
        )
