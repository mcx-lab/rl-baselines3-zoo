...
from stable_baselines.common.distributions import make_proba_dist_type, spaces, DiagGaussianProbabilityDistributionType
...
class DiagGaussianFixedVarProbabilityDistributionType(DiagGaussianProbabilityDistributionType):
    ...
    def proba_distribution_from_latent(
        ...
        logstd = tf.get_variable(
            name="pi/logstd", shape=[1, self.size], initializer=tf.constant_initializer(np.log(pi_init_std)), trainable=False
        )
        ...
        **kwargs
        ...
        if layers is not None:
            ...
            if net_arch is not None:
                warnings.warn("The new `net_arch` parameter overrides the deprecated `layers` parameter!", DeprecationWarning)
