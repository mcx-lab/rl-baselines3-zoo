        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    # pnew / pold
                    ratio = tf.exp(
                        self.policy_pi.proba_distribution.logp(action_ph) - old_pi.proba_distribution.logp(action_ph)
                    )
                    ...
                    self.assign_old_eq_new = tf_util.function(
                        ...
                        updates=[
                            tf.assign(oldv, newv)
                            for (oldv, newv) in zipsame(tf_util.get_globals_vars("oldpi"), tf_util.get_globals_vars("model"))
                        ],
                    )
    ...
    def learn(
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(
            ...
            assert issubclass(self.policy, ActorCriticPolicy), (
                "Error: the input policy for the PPO1 model must be " "an instance of common.policies.ActorCriticPolicy."
            )
            ...
            with self.sess.as_default():
                ...
                while True:
                    ...
                    dataset = Dataset(
                        dict(ob=observations, ac=actions, atarg=atarg, vtarg=tdlamret), shuffle=not self.policy.recurrent
                    )
                    ...
                    for k in range(self.optim_epochs):
                        ...
                        for i, batch in enumerate(dataset.iterate_once(optim_batchsize)):
                            steps = (
                                self.num_timesteps + k * optim_batchsize + int(i * (optim_batchsize / len(dataset.data_map)))
                            )
                            if writer is not None:
                    ...
                    for batch in dataset.iterate_once(optim_batchsize):
                        newlosses = self.compute_losses(
                            batch["ob"], batch["ob"], batch["ac"], batch["atarg"], batch["vtarg"], cur_lrmult, sess=self.sess
                        )
                        losses.append(newlosses)
