...
def imitation_terminal_condition(env, dist_fail_threshold=1.0, rot_fail_threshold=0.5 * np.pi):
    ...
    root_pos_ref, root_rot_ref = pyb.getBasePositionAndOrientation(task.get_ref_model())
    ...
    root_pos_fail = root_pos_diff.dot(root_pos_diff) > dist_fail_threshold * dist_fail_threshold

    root_rot_diff = transformations.quaternion_multiply(
        np.array(root_rot_ref), transformations.quaternion_conjugate(np.array(root_rot_sim))
    )
    ...
    done = motion_over or contact_fall or root_pos_fail or root_rot_fail
