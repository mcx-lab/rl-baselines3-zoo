...
from motion_imitation.utilities import motion_data, motion_util, pose3d
...
class ImitationTask(object):
    ...
    def reward(self, env):
        ...
        if self._imit_mode == "full":
            return self._get_full_reward()
        elif self._imit_mode == "partial":
            return self._get_partial_reward()
        else:
            raise ValueError(f"Mode {self.imit_mode} not recognized")

    def _get_full_reward(self):
    ...
    def _get_partial_reward(self):
        root_velocity_reward = self._calc_reward_root_velocity()
        reward = self._root_velocity_weight * root_velocity_reward
        return reward * self._weight
    ...
    def _calc_reward_end_effector(self):
        ...
        for j in range(num_joints):
            ...
            if is_end_eff:
                ...
                rel_end_pos_ref = pose3d.QuaternionRotatePoint(rel_end_pos_ref, inv_heading_rot_ref)

                rel_end_pos_sim = end_pos_sim - root_pos_sim
                rel_end_pos_sim = pose3d.QuaternionRotatePoint(rel_end_pos_sim, inv_heading_rot_sim)
    ...
    def _calc_reward_root_pose(self):
        ...
        root_rot_diff = transformations.quaternion_multiply(root_rot_ref, transformations.quaternion_conjugate(root_rot_sim))
        _, root_rot_diff_angle = pose3d.QuaternionToAxisAngle(root_rot_diff)
        root_rot_diff_angle = motion_util.normalize_rotation_angle(root_rot_diff_angle)
    ...
    def _calc_reward_root_velocity(self):
        ...
        root_velocity_reward = np.exp(-self._root_velocity_err_scale * root_velocity_err)
    ...
    def _load_ref_motions(self, filenames):
        ...
        logging.info("Loaded {:d} motion clips with {:.3f}s of motion data.".format(num_files, total_time))
    ...
    def _build_ref_model(self):
        ...
        assert num_joints == num_joints_sim, "ref model must have the same number of joints as the simulated model."

        for j in range(num_joints):
            pyb.setCollisionFilterGroupMask(ref_model, j, collisionFilterGroup=0, collisionFilterMask=0)
    ...
    def _update_ref_motion(self):
        ...
        if (self._enable_cycle_sync and (new_phase < self._prev_motion_phase)) or change_clip:
            self._sync_ref_origin(sync_root_position=True, sync_root_rotation=change_clip)
    ...
    def _get_joint_pose_size(self, j):
        ...
        assert pose_size == 1 or pose_size == 0, "Only support 1D and 0D joints at the moment."
        return pose_size
    ...
    def _get_joint_vel_size(self, j):
        ...
        assert vel_size == 1 or vel_size == 0, "Only support 1D and 0D joints at the moment."
        return vel_size

    def get_pose_size(self):
        """Get the total size of a pose array."""
        num_joints = self._get_num_joints()
        pose_size = self._get_joint_pose_idx(num_joints - 1) + self._get_joint_pose_size(num_joints - 1)
        return pose_size

    def get_vel_size(self):
        """Get the total size of a velocity array."""
        num_joints = self._get_num_joints()
        vel_size = self._get_joint_vel_idx(num_joints - 1) + self._get_joint_vel_size(num_joints - 1)
        return vel_size
    ...
    def _calc_ref_pose(self, time, apply_origin_offset=True):
        ...
        motion = self.get_active_motion()
        enable_warmup_pose = self._curr_episode_warmup and time >= -self._warmup_time and time < 0.0
        ...
        if apply_origin_offset:
            ...
            root_rot = transformations.quaternion_multiply(self._origin_offset_rot, root_rot)
    ...
    def _calc_ref_vel(self, time):
        ...
        root_ang_vel = pose3d.QuaternionRotatePoint(root_ang_vel, self._origin_offset_rot)
    ...
    def _calc_ref_pose_warmup(self):
        ...
        delta_heading_rot = transformations.quaternion_about_axis(delta_heading, [0, 0, 1])

        default_root_pos = pose3d.QuaternionRotatePoint(default_root_pos, delta_heading_rot)
        default_root_rot = transformations.quaternion_multiply(delta_heading_rot, default_root_rot)
    ...
    def _sync_ref_origin(self, sync_root_position, sync_root_rotation):
        ...
        if sync_root_rotation:
            ...
            self._origin_offset_rot = transformations.quaternion_about_axis(heading_diff, [0, 0, 1])
    ...
    def _calc_heading(self, root_rotation):
        ...
        """
        inv_default_rotation = transformations.quaternion_conjugate(self._get_default_root_rotation())
        rel_rotation = transformations.quaternion_multiply(root_rotation, inv_default_rotation)
