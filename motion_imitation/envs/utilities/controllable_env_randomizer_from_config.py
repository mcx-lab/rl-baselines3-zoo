...
import numpy as np
from motion_imitation.envs.utilities import controllable_env_randomizer_base, minitaur_env_randomizer_config
...
class ControllableEnvRandomizerFromConfig(controllable_env_randomizer_base.ControllableEnvRandomizerBase):
    ...
    def __init__(self, config=None, verbose=True, param_bounds=(-1.0, 1.0), randomization_seed=None):
        if config is None:
            config = "all_params"
        try:
    ...
    def set_env_from_randomization_parameters(self, env, randomization_parameters):
        ...
        for param_name, random_range in self._randomization_param_dict.items():
            self._randomization_function_dict[param_name](
                lower_bound=random_range[0], upper_bound=random_range[1], parameters=randomization_parameters[param_name]
            )
